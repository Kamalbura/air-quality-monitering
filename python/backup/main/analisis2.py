...
# Redirects to the consolidated python/analysis.py with extended=True 
...
if __name__ == "__main__": 
    # Forward arguments to main script with extended flag
    args = sys.argv[1:] 
    csv_path = args[0] if len(args) > 0 else None 
    start_date = args[1] if len(args) > 1 else None 
    end_date = args[2] if len(args) > 2 else None 
    result = analyze_air_quality_data(csv_path, start_date, end_date, extended=True) 
    import json 
